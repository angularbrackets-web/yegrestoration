...
> the next one is appended — this one said "read its §9" while §10 was current.
> Read the last one, then work backwards.**
> ⚠️ **And do not trust THIS file's summary of BK-53's state.** It has been two
> revisions behind twice. **The ticket's own `Status:` line is the only current
> answer** — `head -20 docs/booking/tickets/BK-53.md`.
...
2. ⚠️ **A KEY FILE IS IMPOSSIBLE HERE** — the org policy `constraints/iam.disableServiceAccountKeyCreation` blocks it. **Do not try; it writes a 0-byte stub and fails.** Impersonation is the supported path and needs no key.
3. **User runs:** `gcloud auth application-default login --impersonate-service-account=ga4-mcp@project-2671172b-3f2b-433f-9b1.iam.gserviceaccount.com` *(no `--scopes` — the blocked scope is never requested for the user identity)*.
4. **User adds that service-account email as a VIEWER in GA4 → Admin → Property access management.** Without this the API returns 403, not `invalid_grant` — a different failure that must not be mistaken for this one recurring.
5. **Reconnect the MCP server** — it does not pick up new credentials on its own. `GOOGLE_APPLICATION_CREDENTIALS` already points at `~/.config/gcloud/application_default_credentials.json`, which is what the impersonation flow rewrites, so no config change is needed.
...
>    ⇄ **GATE 1 is TWO TICKETS since decision 24 — BK-53 (web) and BK-55 (email).** *(This line said "BK-53 is the only blocker" and was stale from 2026-09-05.)* Any text elsewhere calling
...
- **THE RESEND BUTTON RE-RENDERS TODAY'S TERMS ONTO YESTERDAY'S CUSTOMER.**
  ...
  **The fix is not a flag:** row-gate the terms block on `settled.totalCents > 0`,
  ...
  irreversible half worsens with every booking taken.** **Owner: 🔄 BK-55** — moved by decision 24's split (2026-09-05); BK-53 kept the WEB surfaces and no longer claims the resend defect. *(This line said BK-53 for a day after the split.)*
...
| BK-58 | 🔴 **Two false trust claims are live on EVERY page** — the firm was founded **2026**, not 2008 *(decision 38)*, and **BBB Accredited is unsubstantiated** *(decision 39)*. Plus the `Hours` block's *"Open 24/7"* against a 30-slot grid *(decision 35)*. ⛔ ~~16 source sites~~ **SEVENTEEN.** 🔴 **`foundingDate` and the BBB badge are on 17 of 17 built pages** — the claim has TWO grammars and only one is prose | ✅ **DEPLOYED AND VERIFIED LIVE 2026-09-10** (`0c058ae`). Gates green, **six red-first rows observed**, `curl`-verified over ten pages in BOTH grammars. Tier **Light**. 🔴 **SEVENTEEN sites, not sixteen — `llms.txt` was missed by a TRUNCATED grep** |
