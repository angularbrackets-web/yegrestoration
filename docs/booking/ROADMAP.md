...
| Ticket | Scope | Tier | Status |
| --- | --- | --- | --- |
| BK-29 | Remove "free assessment" site-wide (~50 places: every service page, `data/services.ts` SEO copy + FAQs, `about`, `insurance-claims`, `Navbar`, `Layout`'s default meta description, `BlogLayout`, 3 blog posts, `llms.txt`, `TestimonialsSection`, `ServiceHero`, `VideoReel`, `ContactForm`, `contact.astro`) — **plus `book.astro`'s `<title>` and WebPage schema `name`**, which BK-27's revision deliberately left for this ticket so the search surface changes in one commit | Light (copy) | **drafted 2026-08-15** (`tickets/BK-29.md`) — **hard blocker on BK-27's deploy**, not a follow-up. Inventory: **51 claims / 22 files**, grouped into 5 classes. Worst instance is `book/confirmed.astro` — "Your free on-site restoration assessment is booked", shown immediately after the customer ticks the fee acknowledgment. **Its gate must read `dist/`, not source**: `<Navbar />` renders the phrase 7× into `/book/` without `book.astro` containing it once. Key design rule: the figures never leave `booking-copy.ts`, so marketing surfaces drop the claim rather than restating the price. BK-27 already cleared the two strings *inside* the booking surfaces and pinned both templates |
