...
| BK-58 | 🔴 **Two false trust claims are live on EVERY page** — the firm was founded **2026**, not 2008 *(decision 38)*, and **BBB Accredited is unsubstantiated** *(decision 39)*. Plus the `Hours` block's *"Open 24/7"* against a 30-slot grid *(decision 35)*. **16 source sites.** 🔴 **`foundingDate` and the BBB badge are on 17 of 17 built pages** — the claim has TWO grammars and only one is prose | 📝 **DRAFT — the copy in `§3` is PROPOSED, NOT APPROVED.** Tier **Light**, gates only, no adversarial pass. **Ships on `main`, its own commit, NOT with the flip** |
